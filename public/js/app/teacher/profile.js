...
requirejs(['jquery', 'restfulClient', 'leftMenu', 'headMenu', 'layerWrapper', 'upload','darktooltip', 'jqueryExt'],
    function ($, $http, leftMenu, headMenu, layer, upload,darktooltip) {
        leftMenu.init();
        headMenu.init();
        upload.imgUploader({
            onComplete: function (file, done) {
                $('img.avatar').attr('src', file.url);
                var url = '/api/v1/teachers/' + GLOBAL.user._id.toString() + '?me=true';
        ...
        $('#change-psw-btn').click(function () {
            ...
            $http.put('/api/v1/auth/modifyPassword', data, function () {
                layer.msg('修改密码成功!');
            });
        });
