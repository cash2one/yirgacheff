...
requirejs(['jquery', 'restfulClient', 'leftMenu', 'headMenu', 'layerWrapper', 'underscore', 'upload', 'pagination', 'lightbox', 'easyDropDown'],
    function ($, $http, leftMenu, headMenu, layer, _, upload, pagination) {
        leftMenu.init();
        headMenu.init();
        upload.imgUploader({
            multi: true,
        ...
        $('#group-list').on('click', '.select-item', function () {
            ...
            $http.get(url, function (medias) {
                ...
                initPaging();
                var total = pagination.totalNum('list-materials', 'materials-count');
                if (total > 0) {
                    pagination.paginate('list-materials', total, 12, 'pagination');
                }
            });
        });
        ...
        //分页
        function initPaging() {
            $('#list-materials').find('li.data').each(function (index) {
